...
//                    canvas.drawRoundRect(new RectF(x - xValueRect.width() / 2 - dpToPx(3),
//                            yOri + xylinewidth + dpToPx(1), x + xValueRect.width() / 2 + dpToPx(3),
//                            yOri + xylinewidth + dpToPx(2) + xValueRect.height() + dpToPx(2), dpToPx(2)),
//                            dpToPx(2), xyTextPaint);

                    //drawRoundRect(new RectF(0, 0, width, height), mRadius, mRadius, mPaint)

                    canvas.drawRoundRect(new RectF(x - xValueRect.width() / 2 - dpToPx(3),
                                    yOri + xylinewidth + dpToPx(1),
                                    x + xValueRect.width() / 2 + dpToPx(3),
                                    yOri + xylinewidth + dpToPx(2) + xValueRect.height() + dpToPx(2)),
                            dpToPx(2),
                            yOri + xylinewidth + dpToPx(2) + xValueRect.height() + dpToPx(2), xyTextPaint);
                } else {
                    canvas.drawText(text, 0, text.length(), x - rect.width() / 2, yOri + xylinewidth + dpToPx(2) + rect.height(), xyTextPaint);
                }
            }
